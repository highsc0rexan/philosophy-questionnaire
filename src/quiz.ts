...
export const INTRO_RELIGION = `Regarding religion, I have decided to omit all religions except Buddhism, as the majority of what it preaches is, (in my opinion), not inherently religious. Religion has aspects of philosophy, but is not inherently a philosophy in and of itself. Additionally, those who are religious do not need a survey to confirm their own beliefs, so doing so would be pointless. The most it would do is tell you what you already knew, and the worst would be offending someone about their beliefs.`;
